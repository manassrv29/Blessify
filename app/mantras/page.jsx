    ...
    return (<div className="container mx-auto px-4 py-12">
      <div className="max-w-4xl mx-auto">
        ...
        <Tabs defaultValue="daily" className="space-y-6">
          ...
          {Object.entries(mantras).map(([categoryId, categoryMantras]) => (<TabsContent key={categoryId} value={categoryId} className="space-y-4">
              {categoryMantras.map((mantra) => (<Card key={mantra.id} className="overflow-hidden transition-all duration-300 hover:shadow-md">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start">
                      <p className="text-lg">{mantra.text}</p>
                      <Button variant="ghost" size="icon" onClick={() => toggleFavorite(mantra.id)} className={favoriteIds.includes(mantra.id) ? "text-red-500" : ""}>
