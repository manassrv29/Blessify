...
export default function MemoryDetailPage() {
  ...
  useEffect(() => {
    async function fetchMemory() {
      ...
      try {
        const memoryData = await getMemory(params.id as string)
        if (memoryData) {
                ...
                {memory.imageUrls.length > 1 && (
                  <div className="mt-4 grid grid-cols-5 gap-2">
                    {memory.imageUrls.map((url, index) => (
                      <div
                        key={index}
                        className={`cursor-pointer rounded-md overflow-hidden h-16 relative ${
                          url === selectedImage ? "ring-2 ring-primary" : ""
                        }`}
